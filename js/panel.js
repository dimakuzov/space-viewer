...
export class PanelObject {
    ...
    updateTexture() {
        ...
        // Draw border - different color if URL is present
        const borderColor = this.url && this.url.trim() ? 'rgba(76, 175, 80, 1.0)' : 'rgba(255, 255, 255, 1.0)';
        ...
        // If there's a URL, leave space at the bottom for the link indicator
        if (this.url && this.url.trim()) {
            startY = Math.max(30, startY - 15);
        }
        ...
        // Draw URL indicator if URL exists
        if (this.url && this.url.trim()) {
            this.textCtx.font = '20px Montserrat, Arial, sans-serif';
            this.textCtx.fillStyle = 'rgba(76, 175, 80, 1.0)';
    ...
    setUrl(newUrl) {
        // Limit URL length and validate basic format
        this.url = newUrl.substring(0, this.maxUrlLength);
        this.updateTexture();
    }
